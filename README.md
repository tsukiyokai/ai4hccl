...
```
ai4hccl/
├── guide.md          AI辅助研发效率提升全流程指导书(v1.2)
├── setup.sh          一键安装skills到Claude Code
├── docs/             HCCL & AI4SE技术文档、论文(submodule)
└── skills/
    ├── vibe-design/  设计文档生成skill(submodule)
    ├── vibe-review/  代码审查skill(submodule)
    └── vibe-pr/      CANN社区自主开发bot(submodule)
```
...
运行安装脚本，将skills软链接到 `~/.claude/skills/`：
...
```bash
# 创建软链接（vibe-design、vibe-pr）
ln -s "$(pwd)/skills/vibe-design" ~/.claude/skills/vibe-design
ln -s "$(pwd)/skills/vibe-pr"     ~/.claude/skills/vibe-pr

# vibe-review目录结构特殊，需指向内层
ln -s "$(pwd)/skills/vibe-review/skills/vibe-review" ~/.claude/skills/vibe-review

# 或通过npm安装vibe-review（二选一）
...
> /vibe-review https://github.com/.../pull/42    # 审查PR
> /vibe-design                                   # 根据需求生成设计文档（交互式）
> /vibe-pr                                       # 从issue到PR合入的全流程自动化
```
